    ...
    <div className="max-w-7xl mx-auto w-full px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="flex flex-wrap gap-2 border-b border-slate-900 pb-4">
        {[
          ...
          { id: 'transactions', label: 'Transactions', icon: Landmark, count: transactions.filter(t => t.status === 'pending').length },
          { id: 'users', label: 'Joueurs & Activités', icon: Users },
          { id: 'competitions', label: 'Compétitions', icon: Trophy },
          { id: 'videos', label: 'Tutoriels Vidéos', icon: Film },
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all relative cursor-pointer ${
                isActive 
                  ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-655/20' 
            ...
            >
              ...
              {tab.count > 0 && (
                <span className={`ml-1.5 px-2 py-0.5 text-[10px] font-bold rounded-full ${
                  isActive ? 'bg-white text-indigo-600' : 'bg-red-500/20 text-red-400 border border-red-500/30'
                }`}>
                  {tab.count}
                </span>
              )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
          ...
          <div className="glass-panel p-5 rounded-2xl relative overflow-hidden bg-gradient-to-br from-indigo-950/30 to-purple-950/20 border border-indigo-900/30 flex flex-col justify-between">
            ...
            <button
              ...
              className="mt-4 text-[9px] bg-red-950/40 hover:bg-red-900/40 text-red-400 border border-red-500/20 px-2 py-0.5 rounded transition-all font-bold uppercase tracking-wider w-fit cursor-pointer"
            >
              Reset
            </button>
          </div>
                        ...
                        {tx.status === 'pending' ? (
                          <div className="flex justify-end space-x-1.5">
                            <button
                              ...
                              className="p-1.5 bg-emerald-600/10 hover:bg-emerald-600 text-emerald-400 hover:text-white rounded-lg border border-emerald-500/20 transition-all cursor-pointer"
                              title="Approuver"
                            >
                            ...
                            <button
                              ...
                              disabled={actionLoading !== null}
                              className="p-1.5 bg-red-600/10 hover:bg-red-600 text-red-400 hover:text-white rounded-lg border border-red-500/20 transition-all cursor-pointer"
                              title="Refuser"
                            >
          ...
          {transactions.length > visibleTransactionsCount && (
            <div className="flex justify-center pt-2">
              <button
                ...
                className="bg-slate-950 hover:bg-slate-900 text-slate-300 font-bold px-6 py-2.5 rounded-xl text-xs border border-slate-800 hover:border-slate-700 transition-all uppercase tracking-wider shadow-sm active:scale-95 cursor-pointer"
              >
                Voir Plus
              </button>
            </div>
        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-900 pb-3 gap-4">
              ...
              <form onSubmit={handleSearchUserBets} className="flex space-x-2 w-full sm:w-auto">
                ...
                <button
                  ...
                  className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white px-4 py-2 rounded-xl text-sm font-bold transition-all cursor-pointer"
                >
          ...
          <div className="glass-panel p-6 rounded-3xl space-y-4">
            <h3 className="font-display font-black text-lg text-slate-200 border-b border-slate-900 pb-3 flex items-center space-x-2">
              ...
              <span>Gestion Utilisateurs & Modération</span>
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {users.filter(u => u.role !== 'admin').map((u, idx) => (
                <div key={idx} className="flex justify-between items-center bg-slate-950/40 p-4 rounded-xl border border-slate-900/60 hover:border-slate-800 transition-all">
                  <div className="flex flex-col overflow-hidden mr-2">
                    <span className="text-xs font-bold text-slate-300 truncate" title={u.email}>{u.email}</span>
                    <span className="text-[10px] font-mono text-emerald-450 font-bold mt-0.5">{u.balance.toFixed(2)} HTG</span>
                  </div>

                  <button
                    onClick={() => handleToggleSuspend(u.id)}
                    className={`py-1.5 px-3 rounded-lg text-[10px] font-bold flex items-center space-x-1.5 transition-all cursor-pointer shrink-0 ${
                      u.is_suspended 
                        ? 'bg-red-650 hover:bg-red-600 text-white shadow-md shadow-red-500/10'
            ...
            {compConfigs.map(cfg => (
              <div key={cfg.key} className="bg-slate-950/40 p-5 rounded-2xl border border-slate-900 flex flex-col justify-between gap-4">
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-indigo-650/10 hover:bg-indigo-600 text-indigo-455 hover:text-white font-bold rounded-xl text-xs border border-indigo-500/20 transition-all text-center cursor-pointer"
                >
        ...
        <div className="fixed inset-0 bg-slate-950/95 flex items-center justify-center p-4 z-50 backdrop-blur-md">
          <form onSubmit={handleSaveConfig} className="glass-panel p-6 rounded-3xl max-w-lg w-full relative space-y-4 max-h-[90vh] overflow-y-auto">
            <button
              ...
              className="absolute top-4 right-4 text-slate-500 hover:text-slate-300 text-lg font-bold p-2 hover:bg-slate-800 rounded-full h-8 w-8 flex items-center justify-center transition-colors cursor-pointer"
            >
              ✕
            </button>
            ...
            <button
              type="submit"
              className="w-full py-3 bg-indigo-650 hover:bg-indigo-600 text-white font-bold rounded-xl text-sm transition-all cursor-pointer"
            >
              Enregistrer
            </button>
          </form>
        ...
        <div className="glass-panel p-6 rounded-3xl space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-slate-950/40 p-5 rounded-2xl border border-slate-900 space-y-4">
              <h4 className="font-bold text-slate-300 text-sm">Ajouter un nouveau tutoriel</h4>
            ...
            <div className="lg:col-span-2 space-y-4">
              <h4 className="font-bold text-slate-300 text-sm">Vidéos publiées</h4>
